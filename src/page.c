...
static size_t append_dossier_script(char *out, size_t cap, size_t used) {
  used = appendf(
      ...
      "let collapseTimer=0;"
      "let closeTimer=0;"
      "if(!layer||!shell||!main||!closeButton||!backdrop||!triggers.length){return;}"
      "function targetRect(){"
...
size_t render_portfolio_html(char *out, size_t out_cap) {
  ...
  used = appendf(
      ...
      ".dossier-body{display:grid;grid-template-columns:minmax(230px,.72fr) minmax(0,1.28fr);gap:32px;margin-top:26px;}"
      ".dossier-section{padding-top:14px;border-top:1px solid var(--line);}"
      ".dossier-rail{display:grid;gap:18px;align-content:start;}"
      ...
      ".dossier-toc-link:hover{color:var(--accent);}"
      ".dossier-paper{display:grid;gap:18px;align-content:start;}"
      ".dossier-paper-section{padding-top:14px;}"
  ...
  used = appendf(
      ...
      "@media (max-width:860px){.hero,.projects,.principles,.dossier-body,.dossier-metrics{grid-template-columns:1fr;}.project-card,.project-card:first-child{grid-column:span 1;min-height:auto;}h1,.dossier-title{max-width:none;}.project-summary{max-width:none;}.dossier-shell{left:8px;top:8px;width:calc(100vw - 16px);height:calc(100vh - 16px);}.dossier-content{padding:16px 16px 20px;}}"
      "@media (prefers-reduced-motion:reduce){html{scroll-behavior:auto}.site-main,.dossier-backdrop,.dossier-content,.dossier-shell,.project-card{transition:none;animation:none}}"
      "</style>"
      "</head>"
      "<body>"
